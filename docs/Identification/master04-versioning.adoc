...
The other variant is indicated with the modifier `-alpha`, where `-` indicates a version 'before' the version identified by the preceding numeric identifier, and `alpha` indicates an 'alpha' development version. The magnitude of the differences a `-alpha` version are indicated by the difference between the 3-part version identifiers of the current artefact and the previously published one on which it is based.
...
[.text-center]
.Typical Paths
image::diagrams/paths.png[id=typical_paths, align="center", width=90%]
