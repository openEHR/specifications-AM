...
|Correct `ASSERTION._variables_` association to refer to `VARIABLE_DECLARATION`. +
 Add `RULE_ELEMENT` class table to specification (reported in https://openehr.atlassian.net/browse/SPECPR-160[SPECPR-160]) +
 Correct `CARDINALITY._is_set_` documentation (reported in https://openehr.atlassian.net/browse/SPECPR-147[SPECPR-147]).
|T Beale
...
|Renamed `ARCHETYPE_INTERNAL_REF` to `C_OBJECT_PROXY` . +
 https://openehr.atlassian.net/browse/SPECAM-9[SPECAM-9]. Renamed `ontology` section to `terminology` and simplified. +
 Remove `CONSTRAINT_REF`, `C_REFERENCE_OBJECT` types; +
 https://openehr.atlassian.net/browse/SPECAM-2[SPECAM-2]. Introduce new archetype structured identification system; +
...
|Remove `C_DOMAIN_TYPE` ; +
 https://openehr.atlassian.net/browse/SPECAM-27[SPECAM-27]. Merge `C_PRIMITIVE_OBJECT` and `C_PRIMITIVE`; +
...
|
|https://openehr.atlassian.net/browse/SPECAM-22[SPECAM-22]. Limit `_assumed_value_` to `C_PRIMITIVE_OBJECT`.
|T Beale, +
 R Chen
|14 Jan 2013

|
|https://openehr.atlassian.net/browse/SPECAM-32[SPECAM-32]. Remove `C_SINGLE_ATTRIBUTE` and `C_MULTIPLE_ATTRIBUTE` classes.
|T Beale, +
 S Garde, +
 S Kobayashi, +
 D Moner, +
...
|
|https://openehr.atlassian.net/browse/SPECAM-26[SPECAM-26]. Add `_any_allowed_` function to `ARCHETYPE_SLOT`.
|T Beale
|18 Aug 2010

|
|https://openehr.atlassian.net/browse/SPECAM-8[SPECAM-8]. Add specialisation semantics to ADL and AOM. Add various attributes and functions to `ARCHETYPE_CONSTRAINT` descendant classes.
...
|
|https://openehr.atlassian.net/browse/SPECAM-1[SPECAM-1]. Change Date, Time etc classes in AOM to `ISO8601_DATE` , `ISO8601_TIME` etc from Support IM.
|T Beale
|20 Jul 2009

|
|https://openehr.atlassian.net/browse/SPECAM-10[SPECAM-10]. Convert `Interval<Integer>` to `MULTIPLICITY_INTERVAL` to simplify specification and implementation.
|T Beale
|
...
|
|https://openehr.atlassian.net/browse/SPECAM-16[SPECAM-16]. Add validity rules to `ARCHETYPE_TERMINOLOGY` . +
https://openehr.atlassian.net/browse/SPECAM-11[SPECAM-11]. `ARCHETYPE_CONSTRAINT` adjustments. +
https://openehr.atlassian.net/browse/SPECAM-17[SPECAM-17]. Add template object model to AM. +
