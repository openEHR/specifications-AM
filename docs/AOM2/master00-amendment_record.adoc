...
|[[latest_issue]]2.1.2
|{openehr_jira}/SPECAM-57[SPECAM-57]. Adjust model of `ASSERTIONs` and rules to use Expression Language Object Model (ELOM).
|T Beale
|[[latest_issue_date]]16 Sep 2018

|2.1.1
|{openehr_jira}/SPECAM-51[SPECAM-51]. Move RM adaptation attributes from BMM to AOM profile. Add to section 10 the new meta-attributes `archetype_parent_class`, `archetype_data_value_parent_class`, `archetype_visualise_descendants_of`.
|T Beale
|10 Apr 2018

|2.1.0
|{openehr_jira}/SPECAM-48[SPECAM-48]. Add VTPL validity rule for templates - consistency of languages for flattening.
|T Beale
|24 Jan 2018

|
|{openehr_jira}/SPECAM-49[SPECAM-49]. Improve `_c_conforms_to()_` and `_c_congruent_to()_` algorithms; added `_c_value_conforms_to()_` and `_c_value_congruent_to()_` to `C_PRIMITIVE_OBJECT`.
|T Beale
|10 Jan 2018

|
|{openehr_jira}/SPECAM-46[SPECAM-46]. `_c_conforms_to()_` should have lambda parameter for model checking.
|P Bos, +
 T Beale
|07 Jan 2018
