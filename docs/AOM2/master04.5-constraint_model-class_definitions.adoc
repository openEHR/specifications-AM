...
specialised_node_sets_conform_to (other: C_ATTRIBUTE): Boolean
    ...
    require
        ...
        other.is_multiple
    do
        Result := for_all parent_obj in other.children | 
                    collective_occurrences_of (parent_obj, other).intersects (parent_obj.occurrences)
    end
    
collective_occurrences_of (parent_object: C_OBJECT; other: C_ATTRIBUTE): Multiplicity_interval
    ...
    do
        ...
        children.do_all (
            agent (child_co, parent_co: C_OBJECT; interval: Multiplicity_interval)
                local
                ...
                do
                    if child_co.node_id_conforms_to (parent_co) then
                        ...
                        -- that of the parent
                        if attached child_co.occurrences as occ then
                            child_occ := occ
                        else
        ...
        if cardinality /= Void and then not cardinality.upper_unbounded then
            Result.upper := Result.upper.min (cardinality.upper)
        end
    end
---------
...
include::{uml_export_dir}/classes/c_temporal_definitions.adoc[]
