...
The primitive constraint for each primitive type may itself be complex. Its Reference Model (RM) type is given by the type of the `_constraint_` accessor in each `C_PRIMITIVE_OBJECT` descendant and is summarised in the following table. 
...
|`C_DATE_TIME`        |`Date_time`             |`List <Interval<Date_time>>` +
                                                OR +
