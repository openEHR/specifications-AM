...
The AOM `rules` package adds two classes based on classes in the `BASE::org::openehr::expressions` package described in the {openehr_expression}[openEHR Expression Language Object Model (ELOM)]. The package is shown below.

[.text-center]
.am.aom2.rules Package
image::{uml_export_dir}/diagrams/AM-aom2.rules.svg[id=rules_package, align="center"]
