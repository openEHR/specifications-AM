...
The AOM `aom2.rules` package adds two classes based on classes in the `LANG::org.openehr.bmm.expressions` package described in the {openehr_bmm}[BMM Expression Model^]. The package is shown below.
...
The class `AOM_CONSTRAINT_EXPRESSION` specialises the class `EL_CONSTRAINT_EXPRESSION`, which is a hook within the BMM expressions for adding 'match' expressions of the form:
