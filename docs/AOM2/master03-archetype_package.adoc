...
[.rule]
*VARCN*: archetype concept validity. For *openEHR code definitions*: the `_node_id_` of the root object of the archetype must be of the form `at0000{.1}*`, where the number of `.1` components equals the specalisation depth, and must be defined in the terminology.
For *original ADL2 code definitions*: the `_node_id_` of the root object of the archetype must be of the form `id1{.1}*`, where the number of `.1` components equals the specalisation depth, and must be defined in the terminology.
