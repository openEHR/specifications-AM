...
a|Term code defining the meaning of the artefact as a whole, and always used as the at-code (openEHR archetypes) or id-code (ADL2 original archetypes) on the root node of the artefact. Must be defined in the `_term_definitions_` property.

h|*1..1*
|*term_definitions*: `Hash<String, Hash<String, <<_archetype_term_class,ARCHETYPE_TERM>>>>`
a|Directory of term definitions as a two-level table. The outer hash keys are language codes, e.g. `"en"`, `"de"`, while the inner hash keys are term codes, e.g. `"at0017"`, `"at0004"` (openEHR archetypes) or `"id17"`, `"at4"` (ADL2 original archetypes).
...
a|For openEHR archetypes: list of all at codes in the terminology., i.e. the 'at' codes in an ADL archetype, which are the `_node_id_` values on `C_OBJECT` descendants. For ADL2 original archetypes: list of all id codes in the terminology., i.e. the 'id' codes in an ADL archetype, which are the `_node_id_` values on `C_OBJECT` descendants.
