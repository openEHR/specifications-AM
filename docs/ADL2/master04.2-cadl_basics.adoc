...
=== Node Identifiers and Codesystems

Node identifier codes appear after all type identifiers in a cADL text.

ADL2.4 onwards supports a choice of node identifier _codesystems_, in order to facilitate transition from ADL1 to ADL2.

- An `at-code codesystem` identical to that used for ADL1, and mandatory for use in openEHR RM environments.

- An 'id-code codesystem` introduced in ADL2.

Each codesystem has a different node-naming and numbering conventions:
...
The rules and use of node identifiers and the different codesystems is described in more detail at <<xxx>>.
