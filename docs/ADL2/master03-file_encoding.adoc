...
In regular expressions (only used in cADL string constraints), there will typically be backslashescaped characters from the above list as well as other patterns like `\s` (whitspace) and `\d` (decimal digit), according to the http://www.perldoc.com/perl5.6/pod/perlre.html[PERL regular expression specification]. These should not be treated as anything other than literal strings, since they are processed by a regular expression parser.
