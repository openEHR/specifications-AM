...
image::{uml_export_dir}/diagrams/ADL-demographics-parties.svg[id=rm_subtypes, align="center"]
...
Coded terms are treated as a primitive type in ADL in order to enable the formalism and tools to work with terminology constraints. Unlike other primitive constraints, terminology constraints may be complex, because they can refer to external resources, either directly or via 'bindings' defined elsewhere in the archeype. This section describes just the syntax representations and relationships between these. The full semantics, including binding and resolution are described in the section <<Terminology Integration>>.

Syntactically, there are two types of terminology constraint expressible in ADL. The first is expressed with an ac-code which refers to a value set which is either defined in the archetype terminology or externally. The second, for convenience, uses a single at-code, in order to address the case of a single term value without requiring a value-set. For the first case, an assumed value in the form of an at-code is allowed, having the same sense as the assumed values of other primitive types already described.

The possibilities are illustrated below.

[source, cadl]
--------
    -- fragment of an archetype about allergic reaction
    
    items ∈ {
        ELEMENT[id11] occurrences ∈ {0..1} ∈ {	-- Reaction agent
            name ∈ {
                DV_CODED_TEXT[id2] ∈ {
                    defining_code ∈ {[at4]} -- single code case: at4|Reaction Agent|
                }
            }
            value ∈ {
                DV_CODED_TEXT[id3] ∈ {
                    defining_code ∈ {[ac1]} -- value-set case: ac1|Type of Agent|
                }
            }
        }
        ELEMENT[id22] occurrences ∈ {0..1} ∈ {	-- Status
            value ∈ {
                DV_CODED_TEXT[id58] ∈ {
                    defining_code ∈ {[ac2; at2]}    -- value set with assumed value case
                                                     -- ac2|Dx status value|; at2|working|
                }
            }
        }
    }
...
In the above, the constraint at the path `items[id11]/name[id2]` is on a `DV_CODED_TEXT`.`_defining_code_` representing the `_name_` of the `ELEMENT`. It is constrained to a single at-code value representing 'Reaction agent' (we assumed this is some preferred name of the institution that created this archetype). The at-code is defined in the terminology part of the archetype, and may have bindings defined there as well. These are described in later sections.

The second variant uses the code `ac1`, which refers to a value set. This is by definition: all ac-codes in ADL refer only to value sets. This is the most common form of terminology constraint - defining possible codes for a codeable value in the model. The code and any bindings are also defined in the terminology section. The last variant just adds an assumed code, where `at2` must be in the value set referred to by `ac1`.




is defined as a the code `[ac1]`, which acts as an internal identifier of a value set. The value set can be defined locally in the archetype terminology, or externally, or both.


The various types of terminology constraint are described below.
