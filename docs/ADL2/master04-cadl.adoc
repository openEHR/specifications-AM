...
image::{uml_export_dir}/diagrams/ADL-demographics-parties.svg[id=ADL-demographics-parties, align="center"]
...
The double-bar ('|xxx|') method of displaying annotations on codes is adopted from the medical terminology <<SNOMED_CT>> and is widely used in the healthcare domain.
...
Patterns based on ISO 8601 can be used to constraint durations in the same way as for Date/time types. The general form of a pattern is:
