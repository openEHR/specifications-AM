...
What we logically want to do is to state a single constraint on a `DV_QUANTITY` that sets the `_magnitude_` range constraint dependent on the `_units_` constraint.
...
A constraint on the openEHR `DV_ORDINAL` type provides another example of the utility of ADL tuples. First, a typical ordinal constraint (a scale of pass:[+, ++, +++]) with just standard ADL:
...
The full description, including binding and resolution is provided in the section <<Terminology Integration>>. This section describes only the syntax for term constraint in the `definition` section of an archetype.
...
A further syntax possibility exists, for use at the point of operational template generation. As described in the Section [[terminology_integration_concrete_codes]], the choice may be made that a specific operational template should use external codes from the archetype bindings (such as from SNOMED CT, ICD10, etc) as the values of some or all coded nodes, rather than using the internal at-codes. 

To express this choice, the operational template's `definition` section contains a modified version of the usual syntax `[at1]` or `[ac1]` in those nodes where an external term from the bindings is to be used. This takes the form `[acN@ttttt]` or `[atN@ttttt]` where `ttttt` is the namespace identifier of a binding in the `terminology` section of the archetype.

Specifying which codable nodes (including 'all' and 'none' options) should have their values substituted by the external codes is assumed to be part of the operational template generator tool. Different terminology bindings may be specified on different nodes of the same archetype, or none at all, allowing for a mixture of external term substitional depending on node.
