...
A further syntax possibility exists, for use at the point of operational template generation. As described in the Section [[terminology_integration_concrete_codes]], the choice may be made that a specific operational template (OPT) should use external codes from the archetype bindings (such as from SNOMED CT, ICD10, etc) as the values of some or all coded nodes, rather than using the internal at-codes. 
