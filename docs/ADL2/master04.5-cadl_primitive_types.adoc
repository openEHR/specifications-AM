...
fragment TZ_PATTERN     : '±' ('hh' | 'HH') (':'? ('mm' | 'MM'))? ;
--------
...
Any of the above may be modified to require a timezone by appending one of the following timezone constraint patterns:
...
Patterns based on ISO 8601 can be used to constraint durations in the same way as for Date/time types. The lexical rule for the pattern is (Antlr4):
