...
ADL2 was initially released with a new id-coded codesystem using id-codes, at-codes and ac-codes.

The primary change in version 2.4 is the introduction of an alternative at-code codesystem, identical to that used in ADL1.

Although a conversion algorithm was developed to enable conversion for ADL1  at-coded archetypes and templates to the new id-coded codesystem, the openEHR implementer community had concerns on the burden and impact and safety risks inherent in converting downstream software artefacts, queries and persisted patient records.

It was felt that the best solution was to allow openEHR-RM based systems and archetypes to continue to use the original ADL1 at-coded codesystem instead of the id-coded codesystem. THis was seen as critical to remove a very significant barrier to transition from ADL1 to ADL2  which in all other respects was widely welcomed by the established openEHR implementer community,

[WARNING]::
**Is this correct??**

Where The new coding system also carries original ADL 2 id-codes in its archetypes as alternative codes.
...
When using the at-coded codesystem, this ADL2.4 release is fully backwards compatible with openEHR data created based on ADL 1.4 archetypes but breaks compatibility with openEHR data created based on ADL 2.0 - ADL 2.3 archetypes.
