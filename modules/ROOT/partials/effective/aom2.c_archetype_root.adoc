...
2+|Inherits: xref:{class_adl_code_definitions}[ADL_CODE_DEFINITIONS], xref:{class_archetype_constraint}[ARCHETYPE_CONSTRAINT], xref:{class_c_object}[C_OBJECT], xref:{class_c_defined_object}[C_DEFINED_OBJECT], xref:{class_c_complex_object}[C_COMPLEX_OBJECT]
...
_Inherited from xref:{class_c_complex_object}[C_COMPLEX_OBJECT]_
