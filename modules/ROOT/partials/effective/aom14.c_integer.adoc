...
|*assumed_value*: `xref:{class_integer}[Integer]` [0..1]
a|The value to assume if this item is not included in data, due to being part of an optional structure.

|*list*: `xref:{class_list}[List]<xref:{class_integer}[Integer]>` [0..1]
a|Set of Integers specifying constraint.

|*range*: `xref:{class_interval}[Interval]<xref:{class_integer}[Integer]>` [0..1]
