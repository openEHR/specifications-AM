...
2+|Inherits: xref:{class_authored_resource}[AUTHORED_RESOURCE]

2+h|*Attributes*

|*uid*: `xref:{class_hier_object_id}[HIER_OBJECT_ID]` [0..1]
...
|*ontology*: `xref:{class_archetype_ontology}[ARCHETYPE_ONTOLOGY]` [1..1]
...
|*short_concept_name* (): `xref:{class_string}[String]` [1..1]
a|The short concept name of the archetype extracted from the archetype_id.

|*version* (): `xref:{class_string}[String]` [1..1]
a|
