[cols="2,3", frame=none, grid=rows]
|===
2+^h|*OPERATIONAL_TEMPLATE*

2+a|Root object of an operational template. An operational template is derived from a `TEMPLATE` definition and the `ARCHETYPEs` and/or `TEMPLATE_OVERLAYs` mentioned by that template by a process of flattening, and potentially removal of unneeded languages and terminologies.

An operational template is used for generating and validating RM-canonical instance data, and also as a source artefact for generating other downstream technical artefacts, including XML schemas, APIs and UI form definitions.

2+|Inherits: xref:{class_archetype}[ARCHETYPE], xref:{class_authored_resource}[AUTHORED_RESOURCE], xref:{class_authored_archetype}[AUTHORED_ARCHETYPE]

2+h|*Attributes*

|*parent_archetype_id*: `xref:{class_string}[String]` [0..1]
a|Archetype reference of the specialisation par-ent of this archetype, if applicable. May take the form of an archetype interface identifier, i.e. the identifier up to the major version only, or can be deeper. +
_Inherited from xref:{class_archetype}[ARCHETYPE]_

|*archetype_id*: `xref:{class_archetype_hrid}[ARCHETYPE_HRID]` [1..1]
a|Identifier of this archetype. +
_Inherited from xref:{class_archetype}[ARCHETYPE]_

|*is_differential*: `xref:{class_boolean}[Boolean]` [1..1]
a|Flag indicating whether this archetype is differential or flat in its contents. Top-level source archetypes have this flag set to True. +
_Inherited from xref:{class_archetype}[ARCHETYPE]_

|*definition*: `xref:{class_c_complex_object}[C_COMPLEX_OBJECT]` [1..1]
a|Root node of the definition of this archetype. +
_Inherited from xref:{class_archetype}[ARCHETYPE]_

|*terminology*: `xref:{class_archetype_terminology}[ARCHETYPE_TERMINOLOGY]` [1..1]
a|The terminology of the archetype. +
_Inherited from xref:{class_archetype}[ARCHETYPE]_

|*rules*: `xref:{class_list}[List]<link:/classes/STATEMENT[STATEMENT] >` [0..1]
a|Rules relating to this archetype. Statements are expressed in first order predicate logic, and usually refer to at least two attributes. +
_Inherited from xref:{class_archetype}[ARCHETYPE]_

|*rm_overlay*: `xref:{class_rm_overlay}[RM_OVERLAY]` [0..1]
a| +
_Inherited from xref:{class_archetype}[ARCHETYPE]_

|*uid*: `xref:{class_uuid}[UUID]` [0..1]
a|Unique identifier of the family of archetypes having the same interface identifier (same major version). +
_Inherited from xref:{class_authored_resource}[AUTHORED_RESOURCE]_

|*original_language*: `xref:{class_terminology_code}[Terminology_code]` [1..1]
a|Language in which this resource was initially authored. Although there is no language primacy of resources overall, the language of original authoring is required to ensure natural language translations can preserve quality. Language is relevant in both the description and ontology sections. +
_Inherited from xref:{class_authored_resource}[AUTHORED_RESOURCE]_

|*description*: `xref:{class_resource_description}[RESOURCE_DESCRIPTION]` [0..1]
a|Description and lifecycle information of the resource. +
_Inherited from xref:{class_authored_resource}[AUTHORED_RESOURCE]_

|*is_controlled*: `xref:{class_boolean}[Boolean]` [0..1]
a|True if this resource is under any kind of change control (even file copying), in which case revision history is created. +
_Inherited from xref:{class_authored_resource}[AUTHORED_RESOURCE]_

|*annotations*: `xref:{class_resource_annotations}[RESOURCE_ANNOTATIONS]` [0..1]
a|Annotations on individual items within the resource, keyed by path. The inner table takes the form of a Hash table of String values keyed by String tags. +
_Inherited from xref:{class_authored_resource}[AUTHORED_RESOURCE]_

|*translations*: `xref:{class_hash}[Hash]<xref:{class_string}[String],xref:{class_translation_details}[TRANSLATION_DETAILS]>` [0..1]
a|List of details for each natural translation made of this resource, keyed by language code. For each translation listed here, there must be corresponding sections in all language-dependent parts of the resource. The `_original_language_` does not appear in this list. +
_Inherited from xref:{class_authored_resource}[AUTHORED_RESOURCE]_

|*adl_version*: `xref:{class_string}[String]` [0..1]
a|ADL version if archetype was read in from an ADL sharable archetype. +
_Inherited from xref:{class_authored_archetype}[AUTHORED_ARCHETYPE]_

|*build_uid*: `xref:{class_uuid}[UUID]` [1..1]
a|Unique identifier of this archetype artefact instance. A new identifier is assigned every time the content is changed by a tool. Used by tools to distinguish different revisions and/or interim snapshots of the same artefact. +
_Inherited from xref:{class_authored_archetype}[AUTHORED_ARCHETYPE]_

|*rm_release*: `xref:{class_string}[String]` [1..1]
a|Semver.org compatible release of the most recent reference model release on which the archetype in its current version is based. This does not imply conformance only to this release, since an archetype may be valid with respect to multiple releases of a reference model. +
_Inherited from xref:{class_authored_archetype}[AUTHORED_ARCHETYPE]_

|*is_generated*: `xref:{class_boolean}[Boolean]` [1..1]
a|If True, indicates that this artefact was machine-generated from some other source, in which case, tools would expect to overwrite this artefact on a new generation. Editing tools should set this value to False when a user starts to manually edit an archetype. +
_Inherited from xref:{class_authored_archetype}[AUTHORED_ARCHETYPE]_

|*other_meta_data*: `xref:{class_hash}[Hash]<xref:{class_string}[String],xref:{class_string}[String]>` [1..1]
a| +
_Inherited from xref:{class_authored_archetype}[AUTHORED_ARCHETYPE]_

|*component_terminologies*: `xref:{class_hash}[Hash]<xref:{class_string}[String],xref:{class_archetype_terminology}[ARCHETYPE_TERMINOLOGY]>` [0..1]
a|Compendium of flattened terminologies of archetypes externally referenced from this archetype, keyed by archetype identifier. This will almost always be present in a template.

|*terminology_extracts*: `xref:{class_hash}[Hash]<xref:{class_string}[String],xref:{class_archetype_terminology}[ARCHETYPE_TERMINOLOGY]>` [0..1]
a|Directory of term definitions as a two-level  table. The outer hash keys are term codes,  e.g. "at4", and the inner hash key are term  attribute names, e.g. "text", "description" etc.

2+h|*Functions*

|*concept_code* (): `xref:{class_string}[String]` +
 +
__post-condition__: `Result.is_equal (definition.node_id)` [1..1]
a|The concept code of the root object of the archetype, also standing for the concept of the archetype as a whole. +
_Inherited from xref:{class_archetype}[ARCHETYPE]_

|*physical_paths* (): `xref:{class_list}[List]<xref:{class_string}[String]>` [1..1]
a|Set of language-independent paths extracted from archetype. Paths obey Xpath-like syntax and are formed from alternations of `C_OBJECT._node_id_` and `C_ATTRIBUTE._rm_attribute_name_` values. +
_Inherited from xref:{class_archetype}[ARCHETYPE]_

|*logical_paths* ( +
lang: `xref:{class_string}[String][1]` +
): `xref:{class_list}[List]<xref:{class_string}[String]>` [1..1]
a|Set of language-dependent paths extracted from archetype. Paths obey the same syntax as physical_paths, but with node_ids replaced by their meanings from the terminology. +
_Inherited from xref:{class_archetype}[ARCHETYPE]_

|*specialisation_depth* (): `xref:{class_integer}[Integer]` +
 +
__post-condition__: `Result = terminology.specialisation_depth` [1..1]
a|Specialisation depth of this archetype; larger than 0 if this archetype has a parent. Derived from terminology.specialisation_depth. +
_Inherited from xref:{class_archetype}[ARCHETYPE]_

|*is_specialised* (): `xref:{class_boolean}[Boolean]` +
 +
__post-condition__: `Result implies parent_archetype_hrid /= Void` [1..1]
a|True if this archetype is a specialisation of another. +
_Inherited from xref:{class_archetype}[ARCHETYPE]_

|*current_revision* (): `xref:{class_string}[String]` +
 +
__Post__: `Result = revision_history.most_recent_version` [1..1]
a|Most recent revision in revision_history if is_controlled else  (uncontrolled) . +
_Inherited from xref:{class_authored_resource}[AUTHORED_RESOURCE]_

|*languages_available* (): `xref:{class_list}[List]<xref:{class_string}[String]>` [1..1]
a|Total list of languages available in this resource, derived from original_language and translations. +
_Inherited from xref:{class_authored_resource}[AUTHORED_RESOURCE]_

|*component_terminology* ( +
an_id: `xref:{class_string}[String][1]` +
): `xref:{class_archetype_terminology}[ARCHETYPE_TERMINOLOGY]` +
 +
__Inv_is_specialised__: `is_specialised` [1..1]
a|

2+h|*Invariants*

2+a|__Invariant_concept_valid__: `terminology.has_term_code (concept_code)` +
_Inherited from xref:{class_archetype}[ARCHETYPE]_

2+a|__Invariant_specialisation_validity__: `is_specialised implies specialisation_depth > 0` +
_Inherited from xref:{class_archetype}[ARCHETYPE]_

2+a|__Original_language_valid__: `code_set (Code_set_id_languages).has_code (original_language.as_string)` +
_Inherited from xref:{class_authored_resource}[AUTHORED_RESOURCE]_

2+a|__Current_revision_valid__: `(current_revision /= Void and not is_controlled) implies current_revision.is_equal ("(uncontrolled)")` +
_Inherited from xref:{class_authored_resource}[AUTHORED_RESOURCE]_

2+a|__Translations_valid__: `translations /= Void implies (not translations.is_empty and not translations.has (orginal_language.code_string))` +
_Inherited from xref:{class_authored_resource}[AUTHORED_RESOURCE]_

2+a|__Description_valid__: `translations /= Void implies (description.details.for_all (d &#124;
translations.has_key (d.language.code_string)))` +
_Inherited from xref:{class_authored_resource}[AUTHORED_RESOURCE]_

2+a|__Languages_available_valid__: `languages_available.has (original_language)` +
_Inherited from xref:{class_authored_resource}[AUTHORED_RESOURCE]_

2+a|__Revision_history_valid__: `is_controlled xor revision_history = Void` +
_Inherited from xref:{class_authored_resource}[AUTHORED_RESOURCE]_

2+a|__Invariant_adl_version_validity__: `valid_version_id (adl_version)` +
_Inherited from xref:{class_authored_archetype}[AUTHORED_ARCHETYPE]_

2+a|__Invariant_rm_release__: `valid_version_id (rm_release)` +
_Inherited from xref:{class_authored_archetype}[AUTHORED_ARCHETYPE]_

2+a|__Description_validity__: `description /= Void` +
_Inherited from xref:{class_authored_archetype}[AUTHORED_ARCHETYPE]_

2+a|__Specialisation_validity__: `is_specialised`

|===
