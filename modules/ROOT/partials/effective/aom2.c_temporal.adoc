...
2+|Inherits: xref:{class_adl_code_definitions}[ADL_CODE_DEFINITIONS], xref:{class_archetype_constraint}[ARCHETYPE_CONSTRAINT], xref:{class_c_object}[C_OBJECT], xref:{class_c_defined_object}[C_DEFINED_OBJECT], xref:{class_c_primitive_object}[C_PRIMITIVE_OBJECT], xref:{class_c_ordered}[C_ORDERED]
...
|*constraint*: `xref:{class_list}[List]<xref:{class_interval}[Interval]<T>>` [0..1]
...
_Inherited from xref:{class_c_ordered}[C_ORDERED]_
