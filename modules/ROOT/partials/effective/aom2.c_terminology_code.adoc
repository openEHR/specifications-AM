...
2+|Inherits: xref:{class_adl_code_definitions}[ADL_CODE_DEFINITIONS], xref:{class_archetype_constraint}[ARCHETYPE_CONSTRAINT], xref:{class_c_object}[C_OBJECT], xref:{class_c_defined_object}[C_DEFINED_OBJECT], xref:{class_c_primitive_object}[C_PRIMITIVE_OBJECT]
...
|*default_value*: `xref:{class_terminology_code}[Terminology_code]` [0..1]
a|
...
|*constraint_status*: `xref:{class_constraint_status}[CONSTRAINT_STATUS]` [0..1]
...
|*valid_value* ( +
a_value: `xref:{class_terminology_code}[Terminology_code][1]` +
): `xref:{class_boolean}[Boolean]` [1..1]
a|True if a `_value_` is valid with respect to constraint expressed in concrete instance of this type.

|*prototype_value* (): `xref:{class_terminology_code}[Terminology_code]` [1..1]
...
|*c_value_conforms_to* ( +
other: `xref:{class_c_terminology_code}[C_TERMINOLOGY_CODE][1]` +
): `xref:{class_boolean}[Boolean]` [1..1]
...
|*constraint_required* (): `xref:{class_boolean}[Boolean]` [1..1]
...
|*effective_constraint_status* (): `xref:{class_integer}[Integer]` [1..1]
...
|*value_set_expanded* (): `xref:{class_list}[List]<xref:{class_string}[String]>` [0..1]
...
|*value_set_substituted* (): `xref:{class_list}[List]<xref:{class_uri}[Uri]>` [0..1]
...
|*value_set_resolved* (): `xref:{class_list}[List]<xref:{class_terminology_code}[Terminology_code]>` [0..1]
