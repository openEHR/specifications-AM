...
h|*1..1*
|*interval*: `xref:{class_interval}[Interval]<xref:{class_integer}[Integer]>`
a|The interval of this cardinality.

h|*1..1*
|*is_ordered*: `xref:{class_boolean}[Boolean]`
a|True if the members of the container attribute to which this cardinality refers are ordered.

h|*1..1*
|*is_unique*: `xref:{class_boolean}[Boolean]`
