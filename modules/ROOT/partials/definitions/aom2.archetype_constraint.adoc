...
h|*Inherit*
2+|`xref:{class_adl_code_definitions}[ADL_CODE_DEFINITIONS]`
...
h|*0..1*
|*parent*: `xref:{class_archetype_constraint}[ARCHETYPE_CONSTRAINT]`
a|Parent node, except in the case of the top of a tree, i.e. root `C_COMPLEX_OBJECT` of an archetype definition.

h|*0..1*
|*soc_parent*: `xref:{class_c_second_order}[C_SECOND_ORDER]`
a|
...
h|*1..1*
|*is_root* (): `xref:{class_boolean}[Boolean]`
a|True if this node is the root of the tree.

h|*1..1*
|*is_leaf* (): `xref:{class_boolean}[Boolean]`
