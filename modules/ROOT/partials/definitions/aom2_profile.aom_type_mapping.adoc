...
h|*1..1*
|*source_class_name*: `xref:{class_string}[String]`
a|Name of the AOM type being mapped to an RM type.

h|*1..1*
|*target_class_name*: `xref:{class_string}[String]`
a|Name of the RM type in the mapping.

h|*0..1*
|*property_mappings*: `xref:{class_hash}[Hash]<xref:{class_string}[String],xref:{class_aom_property_mapping}[AOM_PROPERTY_MAPPING]>`
