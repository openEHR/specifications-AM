...
|*Temporal types*   |`List <Interval<T->ISO8601_TYPE>>` +
                     OR +
                    `String` (ADL pattern)      |As for ordered types, with T being an ISO8601-based type, with the addition of a second type constraint - {openehr_am_adl2}#_constraints_on_dates_times_and_durations[a pattern based on ISO8601 syntax].
...
include::ROOT:partial$classes/aom2.cardinality.adoc[]
